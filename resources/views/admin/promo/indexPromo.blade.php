    ...
    <div>
        <p class="mt-4 text-2xl font-bold text-gray-900 dark:text-white">
            Buat promo yang menarik mulai sekarang!
        </p>
    </div>
    ...
    <div class="flex flex-col mt-5">
        ...
        <div class="inline-block min-w-full align-middle">
            <div class="relative mb-4 text-gray-500 focus-within:text-gray-900">
                <div class="absolute inset-y-0 flex items-center pl-3 pointer-events-none left-1 ">
                ...
                <input type="text" id="default-search" class="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Cari promo">
            </div>
            <div class="overflow-hidden ">
                <table class="min-w-full rounded-xl">
                    <thead>
                        <tr class="bg-gray-50">
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize rounded-tl-xl"> Title </th>
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize"> Price </th>
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize"> Actual Price </th>
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize"> Description </th>
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize"> Status Display </th>
                            <th scope="col" class="p-5 text-sm font-semibold leading-6 text-left text-gray-900 capitalize rounded-tr-xl"> Actions </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-300" id="listPromo">
                    </tbody>
                </table>
            </div>
        </div>
    ...
    // Update Promo
        ...
        document.addEventListener("DOMContentLoaded", () => {
            const tableBody = document.getElementById("listPromo");
            const fetchPromoData = async () => {
                ...
                data.forEach((promo) => {
                    ...
                    <tr class="transition-all duration-500 bg-white hover:bg-gray-50">
                        <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap">${promo.title}</td>
                        <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap">${promo.price}</td>
                        ...
                        <td class="p-5 text-sm font-medium leading-6 text-gray-900 whitespace-nowrap">
                            <button 
                                class="w-full px-3 py-2 text-xs text-white font-medium rounded-full ${promo.ondisplay ? 'bg-green-500' : 'bg-red-500'}">
                                ${promo.ondisplay ? 'Ditampilkan' : 'Tidak Ditampilkan'}
                            </button>
                        </td>
                        <td class="p-5 ">
                            <div class="flex items-center gap-1">
                                <button onclick="updatePromo(${promo.id})" class="flex p-2 transition-all duration-500 rounded-full group item-center">
                                ...
                                <button onclick="deletePromo(${promo.id})" class="flex p-2 transition-all duration-500 rounded-full group item-center">
                ...
                tableBody.innerHTML = "<tr><td colspan='6' class='p-5 text-sm text-center text-red-500'>Data gagal dimuat.</td></tr>";
                }
            };
            fetchPromoData();
        });
