<header class="sticky top-0 z-999 flex w-full bg-landing-body drop-shadow-1 dark:bg-landing-black-1 dark:drop-shadow-none">
    <div class="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        <div class="flex items-center gap-2 sm:gap-4 lg:hidden">
            <button
              class="z-99999 block rounded-sm border border-stroke bg-white p-1.5 shadow-sm dark:border-landing-brown-1 dark:bg-landing-brown-3 lg:hidden"
              @click.stop="sidebarToggle = !sidebarToggle"
            >
            <span class="relative block h-5.5 w-5.5 cursor-pointer">
              <span class="du-block absolute right-0 h-full w-full">
                <span
                  class="relative left-0 top-0 my-1 block h-0.5 w-0 rounded-sm bg-landing-brown-3 delay-[0] duration-200 ease-in-out dark:bg-white"
                  :class="{ '!w-full delay-300': !sidebarToggle }"
                ></span>
                <span
                  class="relative left-0 top-0 my-1 block h-0.5 w-0 rounded-sm bg-landing-brown-3 delay-150 duration-200 ease-in-out dark:bg-white"
                  :class="{ '!w-full delay-400': !sidebarToggle }"
                ></span>
                <span
                  class="relative left-0 top-0 my-1 block h-0.5 w-0 rounded-sm bg-landing-brown-3 delay-200 duration-200 ease-in-out dark:bg-white"
                  :class="{ '!w-full delay-500': !sidebarToggle }"
                ></span>
              </span>
              <span class="du-block absolute right-0 h-full w-full rotate-45">
                <span
                  class="absolute left-2.5 top-0 block h-full w-0.5 rounded-sm bg-landing-brown-3 delay-300 duration-200 ease-in-out dark:bg-white"
                  :class="{ '!h-0 delay-[0]': !sidebarToggle }"
                ></span>
                <span
                  class="delay-400 absolute left-0 top-2.5 block h-0.5 w-full rounded-sm bg-landing-brown-3 duration-200 ease-in-out dark:bg-white"
        ...
        <div class="flex items-center space-x-4 h-10">
          {{-- Toggle mode --}}
            <div>
              <label
                :class="darkMode ? 'bg-landing-brown-3' : 'bg-stroke'"
          ...
          >
            ...
            <div
              x-show="dropdownOpen"
              class="absolute right-0 mt-4 flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-landing-brown-3 dark:bg-landing-black-1"
            >
              <ul
                class="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-landing-brown-3"
              >
                <li>
                  <a
                    href="profile.html"
              ...
              <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button
                  class="flex items-center gap-3 px-6 py-4 text-sm font-medium duration-300 ease-in-out hover:text-landing-brown-1 dark:hover:text-landing-body lg:text-base"
                >
                  <svg
                    class="fill-current"
                    width="22"
